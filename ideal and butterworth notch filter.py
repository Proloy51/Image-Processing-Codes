...
image = cv2.imread('two_noise.jpg', cv2.IMREAD_GRAYSCALE)
...
def calculate_h_nr(u, v, centers, Do, n, ideal):
    h_nr = 1
    for (uk, vk) in centers:
        Dk = ((u - uk) ** 2 + (v - vk) ** 2) ** 0.5
        D_k = ((u + uk) ** 2 + (v + vk) ** 2) ** 0.5
...
Do = int(input('Enter Cutoff frequency: '))
n = int(input('Enter Degree: '))
ideal = int(input('Ideal Notch Reject Filter? (1 for Yes, 0 for No): '))
...
for i in range(M):
    for j in range(N):
        kernel[i, j] = calculate_h_nr(i, j, centers, Do, n, ideal)
...
filtered_ft = np.multiply(filtered_mag, np.exp(1j * ft_ang))
